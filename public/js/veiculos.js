function listarGrupos() {
  fetch(`/veiculos/listarPorEmpresa/${sessionStorage.ID_EMPRESA}`)
...
function editarOnibus(idGrupo, icone_editar) {
  const div_adicionar = document.querySelector(
    `.div_adicionar.grupo${idGrupo}`
  );
  const mostrar =
  ...
  icone_editar.innerHTML = mostrar
    ? `<i class="bi bi-check"></i>`
    : `<i class="bi bi-pencil"></i>`;

  const deleteButtons = document.querySelectorAll(
    `.delete-onibus.grupo${idGrupo}`
  );
